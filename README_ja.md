...
- **入力項目**:
  ...
  - `wildcard_selector` (COMBO): フォルダ内のYAMLファイルから自動スキャンされたワイルドカードキー（例: `sample/gravure_pose/basic_pose` や上位 of `sample/gravure_pose` など）を選択します。
...
指定されたフォルダ内の画像を読み込み、同名のテキストファイル（`.txt`）があればその内容（プロンプト）をセットで出力するノードです。主にImage-to-Video (I2V) などにおいて、入力画像とプロンプトを同期させながら処理を行いたい場合に最適です。
ノード上でのインタラクティブな画像およびテキスト（プロンプト）のリアルタイムプレビュー機能を搭載しています。

- **カテゴリ**: `M4/loaders`
- **入力項目**:
  - `folder_path` (STRING): 画像が保存されているフォルダの絶対パス。
  - `mode` (COMBO): 読み込み順序の制御モード。
    - `increment`: 実行（Queue Prompt）ごとに順番に画像とテキストを読み込みます。起点は `image_select` で選択した画像で、ファイル名順に読み進め、最後まで到達したら自動的に最初に戻ります。
    - `fixed`: 選択された画像を起点に、`fixed_index` で指定されたオフセット位置の画像を固定出力します。
    - `random`: `seed` に基づいて、フォルダ内の画像をランダムに読み込みます。
  - `image_select` (COMBO): フォルダ内の画像ファイル一覧から選択するリスト（フォルダパス入力時に自動更新されます）。
  - `fixed_index` (INT): `fixed` モードの際に対象画像をシフトするオフセット値（0始まり）。
  - `seed` (INT): `random` モードの際に使用する乱数シード。
- **出力項目**:
  - `IMAGE`: 読み込んだ画像データ。
  - `STRING`: 同名テキストファイルの内容（存在しなければ空文字）。
  - `WIDTH` (INT): 画像の幅。
  - `HEIGHT` (INT): 画像の高さ。
